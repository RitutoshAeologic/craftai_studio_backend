...
class GroqClient(ILLMClient):
    def __init__(self, api_key: Optional[str] = None, model: Optional[str] = None):
        ...
        self.model = model or getattr(settings, "GROQ_MODEL", getattr(settings, "LLM_MODEL_NAME", "qwen/qwen3.8-27b"))
    ...
    async def expand_prompt(self, raw_prompt: str, starter_chip: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not self.api_key or self.api_key.startswith("your-"):
            raise LLMExecutionError("Groq API key is not configured.")
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=8.0) as client:
                ...
                if resp.status_code != 200:
                    raise LLMExecutionError(f"Groq expand HTTP {resp.status_code}: {resp.text[:120]}")

                data = resp.json()
                raw_content = data["choices"][0]["message"]["content"]
                content = clean_and_parse_llm_json(raw_content)
                if not content or "master_prompt" not in content:
                    raise LLMExecutionError(f"Groq returned invalid JSON: {raw_content[:100]}")
            ...
            raise LLMExecutionError(f"Groq error: {e}")
    ...
    async def compile_delta(self, base_prompt: str, user_instruction: str) -> Dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=8.0) as client:
                resp = await client.post(self.endpoint, headers=headers, json=payload)
                if resp.status_code != 200:
                    raise LLMExecutionError(f"Groq delta HTTP {resp.status_code}: {resp.text[:120]}")

                data = resp.json()
                raw_content = data["choices"][0]["message"]["content"]
                content = clean_and_parse_llm_json(raw_content)
                if not content or "compiled_prompt" not in content:
                    raise LLMExecutionError(f"Groq delta returned invalid JSON: {raw_content[:100]}")
            ...
            raise LLMExecutionError(f"Groq delta error: {e}")
