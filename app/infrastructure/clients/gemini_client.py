...
class GeminiVisionClient(IVisionClient):
    ...
    async def scan_photo(self, photo_url_or_path: str) -> Dict[str, Any]:
        """Scans image aesthetics, camera optics, and style metadata in non-blocking thread."""
        ...
        # Enterprise fallback
        return {
            "extracted_prompt": "Artistic study inspired by reference, fine art portrait, dramatic chiaroscuro lighting, natural textures, 35mm film grain",
    ...
    async def extract_facial_identity(self, image_bytes: bytes, mime_type: str = "image/jpeg") -> str:
        """Extracts high-dimensional facial identity descriptors in non-blocking worker thread."""
        ...
        try:
            ...
            result = await asyncio.to_thread(self._model.generate_content, [prompt, image_part])
